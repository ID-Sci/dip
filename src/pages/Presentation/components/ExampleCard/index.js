...
function ExampleCard({ image, name, count, pro, ...rest }) {
  const imageTemplate = (
    ...
    >
      {pro && (
        <DIPBox position="absolute" top={0} right={0} zIndex={2} p={1}>
          <svg
            width="24px"
            height="24px"
            viewBox="0 0 24 24"
  ...
  return (
    <DIPBox position="relative">
      ...
      {name || count > 0 ? (
        <DIPBox mt={1} ml={1} lineHeight={1}>
          {name && (
            <DIPTypography variant="h6" fontWeight="bold">
              {name}
            </DIPTypography>
          )}
          {count > 0 && (
            <DIPTypography variant="button" fontWeight="regular" color="secondary">
